...
| **web** | React (PWA) | 内嵌在 relay 二进制里 | 手机端对话 UI + 权限审批 + 状态监控 |

### 数据流

```
手机浏览器 ←WSS→ VPS relay ←WSS→ 本机 bridge ←stdin/stdout→ claude -p (stream-json)
```

- relay 是消息管道，做最小信封解析（auth/control type），不解析对话内容，不碰 API key
- bridge 是纯转发桥接，spawn 原版 claude 进程
- API key 永远不离开用户本机
